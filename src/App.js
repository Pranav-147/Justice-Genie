...
function App() {
  ...
  useEffect(() => {
    const setupDB = async () => {
      ...
      const sessions = await dbInstance.getAllFromIndex('sessions', 'timestampIndex');
  ...
  const startNewChat = async () => {
    try {
      ...
      if (db) {
        await db.add('sessions', { sessionId: newSessionId, timestamp: Date.now() });
      ...
      <div className="sideBar">
        <div className="upperSide">
          <div className="upperSideTop">
            ...
            <span className="brand">Chat With DOJ</span>
          </div>
          ...
          <button
            ...
            onClick={() => window.open('https://doj.gov.in/national-judicial-data-grid-2/', '_blank')}
          >
          ...
          <button
            className="midBtn linkBtn"
            onClick={() => window.open('https://vcourts.gov.in/virtualcourt/', '_blank')}
          >
